	...
	limcounts *sc, limcounts *qc, schd_error *err)
{
	...
	log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
		...
		rr->name.c_str(), user, max_user_run, max_genuser_run, used);
	...
	limcounts *sc, limcounts *qc, schd_error *err)
{
	...
	log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
		...
		rr->name.c_str(), group, max_group_run, max_gengroup_run, used);
	...
	limcounts *sc, limcounts *qc, schd_error *err)
{
	...
	if (ret != 0)
		log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
			   "%s check_max_user_res returned %d", rr->name.c_str(), ret);
	switch (ret) {
		default:
		case -1:
			return (SCHD_ERROR);
	...
	limcounts *sc, limcounts *qc, schd_error *err)
{
	...
	if (ret != 0)
		log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
			"%s check_max_group_res returned %d", rr->name.c_str(), ret);
	switch (ret) {
		default:
		case -1:
			return (SCHD_ERROR);
	...
	limcounts *sc, limcounts *qc, schd_error *err)
{
	...
	for (res = limres; res != NULL; res = res->next) {
		...
		log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
			"%s max_res.%s %.1lf, used %.1lf", rr->name.c_str(), res->name, max_res, used);
...
check_queue_max_user_run_soft(server_info *si, queue_info *qi, resource_resv *rr)
{
	...
	log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
		...
		rr->name.c_str(), user, max_user_run_soft, max_genuser_run_soft, used);
	...
	resource_resv *rr)
{
	...
	log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
		...
		rr->name.c_str(), group, max_group_run_soft, max_gengroup_run_soft, used);
	...
	resdef **rdef, void *limitctx)
{
	...
	for (res = limres; res != NULL; res = res->next) {
		...
		log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
			...
			rr->name.c_str(), group, res->name, max_group_res, max_gengroup_res, used);
...
check_max_group_res_soft(resource_resv *rr, counts *cts_list, void *limitctx, int preempt_bit)
{
	...
	for (res = limres; res != NULL; res = res->next) {
		...
		log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
			...
			rr->name.c_str(), group, res->name, max_group_res_soft, max_gengroup_res_soft, used);
	...
	void *limitctx)
{
	...
	for (res = limres; res != NULL; res = res->next) {
		...
		log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
			...
			rr->name.c_str(), user, res->name, max_user_res, max_genuser_res, used);
	...
	counts *cts_list, void *limitctx, int preempt_bit)
{
	...
	for (res = limres; res != NULL; res = res->next) {
		...
		log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
			...
			rr->name.c_str(), user, max_user_res_soft, max_genuser_res_soft, used);
	...
	resdef **rdef, void *limitctx)
{
	...
	for (res = limres; res != NULL; res = res->next) {
		...
		log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
			...
			rr->name.c_str(), project, res->name, max_project_res, max_genproject_res, used);
...
check_max_project_res_soft(resource_resv *rr, counts *cts_list, void *limitctx, int preempt_bit)
{
	...
	for (res = limres; res != NULL; res = res->next) {
		...
		log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
			...
			rr->name.c_str(), project, res->name, max_project_res_soft, max_genproject_res_soft, used);
	...
	limcounts *sc, limcounts *qc, schd_error *err)
{
	...
	if (ret != 0) {
		log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
			"%s check_max_project_res returned %d", rr->name.c_str(), ret);
	}
	switch (ret) {
		default:
		case -1:
	...
	resource_resv *rr)
{
	...
	log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
		...
		rr->name.c_str(), project, max_project_run_soft, max_genproject_run_soft, used);
	...
	limcounts *sc, limcounts *qc, schd_error *err)
{
	...
	log_eventf(PBSEVENT_DEBUG4, PBS_EVENTCLASS_JOB, LOG_DEBUG, __func__,
		...
		rr->name.c_str(), project, max_project_run, max_genproject_run, used);
